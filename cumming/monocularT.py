...
class MultiDatasetT(SensoryBase):
    """
    ...
    def __init__(self,
        ...
        **kwargs):
        ...
        for f, fhandle in enumerate(self.fhandles):
            ...
            #blocks = (np.sum(fhandle['data_filters'][:,:], axis=0)==0).astype(np.float32)
            ...
            blockstart = np.array(fhandle['block_inds'][0,:],dtype=np.int64)-1 #these loaded indices are the start and end of every trial with no gaps between trials
            blockend = np.array(fhandle['block_inds'][1,:],dtype=np.int64)-1 #-1 for matlab indexing correction
            nblocks = len(blockstart)

            if self.trial_size is None:
                trsize_list = []
                for b in range(nblocks):
                    trsize_list.append(blockend[b]-blockstart[b]+1)#trial size should be start ind - end ind + 1
                self.trial_size = int(np.median(trsize_list))
            for b in range(nblocks):
                self.file_index.append(f)
                self.block_inds.append(self.NT + np.arange(blockstart[b], blockend[b]+1))
        ...
        if self.preload:
            ...
            self.spike_times = np.zeros((0,2)) #will append to this, second dimension is for cell index
            ...
            for f, fhandle in enumerate(self.fhandles):
                ...
                start_times_refs = fhandle['trial_data']['start_times'][0]
                end_times_refs = fhandle['trial_data']['end_times'][0]

                trial_starts = np.zeros(len(start_times_refs))
                trial_ends = np.zeros(len(end_times_refs))

                for i in range(len(start_times_refs)):
                    trial_starts[i] = fhandle[start_times_refs[i]][0][0] #getting data out of mat file structure is strange sometimes
                    trial_ends[i] = fhandle[end_times_refs[i]][0][0]
    ...
    def __getitem__(self, index):
        ...
        if self.preload:
            ...
            for ii in index:
                ...
                """ Stim """
                stim_tmp = torch.tensor(np.transpose(self.fhandles[f]['stimulus'])[inds,:], dtype=torch.float32)

                """ Spikes: needs padding so all are B x NC """ 
                robs_tmp = torch.tensor(np.transpose(self.fhandles[f]['binned_SU'])[inds,:], dtype=torch.float32)
                ...
                dfs_tmp = torch.tensor(np.transpose(self.fhandles[f]['data_filters'])[inds,:], dtype=torch.float32)
