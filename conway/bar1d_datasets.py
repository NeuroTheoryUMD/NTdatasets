...
class BarET(SensoryBase):
    ...
    def __init__(self,
        ...
        datadir, 
        include_MUs=True,
        # Stim setup -- have option to assemble or could leave blank
        ...
        self.stim_location = np.array(self.fhandles[0]['stim_location'], dtype=int)
        self.stim_locationET = np.array(self.fhandles[0]['ETstim_location'], dtype=int)
        ...
        self.channel_mapSU = np.array(self.fhandles[0]['Robs_probe_ID'], dtype=np.int64)[0, :]
        self.channel_mapMU = np.array(self.fhandles[0]['RobsMU_probe_ID'], dtype=np.int64)[0, :]
        ...
        #self.robs = []
        #self.NT = 0
        # Add additional stimuli for processing
        self.stimHout, self.stimVout = None, None
        self.stim_dimsH, self.stim_dimsV = None, None
        self.fix_n = []
        ...
        for fnum, fhandle in enumerate(self.fhandles):
            ...
            print('Stim check:', self.stimname, self.dims)
        ...
        if time_embed is not None:  # then call assemble_stimulus in the constructor
            self.assemble_stimulus(
                combine_stim=combine_stim, stim_crop=stim_crop,
                time_embed=time_embed, num_lags=num_lags,
                Hshift=Hstim_shift, Hdiscardzero=Hdiscardzero)
        # Otherwise leave for the future
    ...
    def preload_numpy(self):
        ...
        # Make combined stim (in case used)
    ...
    def assemble_stimulus(self, 
        combine_stim=None, stim_crop=None,
        ...
        # Assemble/shift horizontal stim
        print( "Assembling stimulus")
        sh = abs(Hshift)
        if sh == 0:
            Hstim = self.stimH
        else:
            Lc = self.NX - sh
            Hstim = np.zeros(self.stimH.shape, dtype=np.float32)
            if Hdiscardzero:
                print('  Discarding stim at 0 horizontal position')
                if Hshift < 0:
                    Hstim[:, range(Lc)] = deepcopy(self.stimH[:, range(sh, self.NX)])
                    Hstim[:, range(Lc, self.NX-1)] = deepcopy(self.stimH[:, range(1,sh)])
                elif Hshift > 0:
                    Hstim[:, range(sh, self.NX-1)] = deepcopy(self.stimH[:, range(1,Lc)])
                    Hstim[:, range(sh)] = deepcopy(self.stimH[:, range(Lc, self.NX)])
            else:
                if Hshift < 0:
                    Hstim[:, range(Lc)] = deepcopy(self.stimH[:, range(sh, self.NX)])
                    Hstim[:, range(Lc, self.NX)] = deepcopy(self.stimH[:, range(sh)])
                elif Hshift > 0:
                    Hstim[:, range(sh, self.NX)] = deepcopy(self.stimH[:, range(Lc)])
                    Hstim[:, range(sh)] = deepcopy(self.stimH[:, range(Lc, self.NX)])
            # Reassemble stimC
            self.stimC[:, range(Hstim.shape[1])] = deepcopy(Hstim)
   
        # Time embed horizontal and vertical stim
        if time_embed == 2:
            print("  Time embedding...")
            idx = np.arange(self.NT)
            tmp_stimH = Hstim[idx[:,None]-np.arange(num_lags), ...]
            tmp_stimV = self.stimV[idx[:,None]-np.arange(num_lags), ...]

            tmp_stimH = np.transpose( tmp_stimH, axes=[0,2,1] )
            tmp_stimV = np.transpose( tmp_stimV, axes=[0,2,1] )
            if self.combine_stim:
                tmp_stimC = self.stimC[idx[:,None]-np.arange(num_lags), ...]
                tmp_stimC = np.transpose( tmp_stimC, axes=[0,2,1] )

        NT = Hstim.shape[0]
        self.stimHout = torch.tensor( tmp_stimH.reshape([NT, -1]), dtype=torch.float32, device=self.device)
        ...
        if self.combine_stim:
            ...
            self.stim = torch.tensor( 
                tmp_stimC.reshape([tmp_stimC.shape[0], -1]), 
                dtype=torch.float32, device=self.device)
        else: 
            self.stim_dims = deepcopy(self.stim_dimsH)
            self.stim = deepcopy(self.stimHout)
...
############ ORIGINAL BAR DATASET -- NOW VERSION 7 ###############
class Bar1Dv7(Dataset):
    """
    -- can load batches from multiple datasets
    -- hdf5 files must have the following information:
        Robs
